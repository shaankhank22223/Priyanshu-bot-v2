...
module.exports = {
  config: {
    ...
    usage: "{prefix}edit [reply image] [prompt]",
  ...
  run: async function({ api, message, args }) {
    const { threadID, messageID, type, messageReply } = message;

    try {
      // Verification: Reply check and photo attachment check
      if (
        type !== "message_reply" ||
      ...
      global.logger.error(`Error in edit command: ${error.message}`);
