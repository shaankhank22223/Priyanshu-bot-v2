...
async function getAiReply(senderID, promptText, senderName, senderGender) {
    const systemPrompt = `Tumhara name Muskan hai, ap ek smart aur cute ladki hai. Tum Shaan ki Girlfriend ho. Koi flirt Kare to usko acchi Tarah se jawab den. Jawab Roman Urdu/Hinglish mein do. User Name: ${senderName}, Gender: ${senderGender}. Rules: 1. Shaan Khan (${OWNER_UID}) tumhare owner aur love hain. 2. Agar sender UID (${senderID}) Shaan hai, to bahut pyar se baat karo (like a wife). 3. Dusro ke liye ek cool/cute friend raho. 4. Short responses (max 3 lines) ✨💖.`.trim();
    ...
    run: async function ({ api, message, args }) {
        ...
        try {
            ...
            if (isVideoReq || isAudioReq || isUrl) {
                ...
                if (!isVideoReq) await api.sendMessage(mediaDetails, threadID);
