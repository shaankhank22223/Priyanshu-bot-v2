...
module.exports = {
          ...
          if (matched) {
            ...
            for (const threadID of allThreads) {
              api.sendMessage(msg, threadID, (err) => {
                if (err) global.logger.error(`AutoSent failed for ${threadID}: ${err.message}`);
              });
            }
          }
        } catch (error) {
          global.logger.error(`Error in AutoSent scheduler: ${error.message}`);
        }
      }, 1000);
      global.logger.system("AutoSent scheduler started successfully");
    }
  },
  ...
  run: async function({ api, message }) {
    ...
    try {
      return api.sendMessage("✅ AutoSent command is active. It will automatically send messages every hour.", threadID, messageID);
    } catch (error) {
      global.logger.error(`Error in autosent run: ${error.message}`);
      return api.sendMessage("❌ Error checking status.", threadID, messageID);
    }
  }
};